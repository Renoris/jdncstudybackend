package sehwan._5_17;

public class AdvancedElixir extends Item implements ElixirPotion{
    int value;

    public AdvancedElixir(String name, String value) {
        super(name);
        this.value = Integer.parseInt(value);
    }

    @Override
    public void increasePotionToxic(Member member) {
        member.setPotionToxic(member.getPotionToxic() + 5);
        System.out.println(member.getName() + "의 포션중독 수치가 " + member.getPotionToxic() + " 만큼 오릅니다.");
    }

    @Override
    public void use(Member member) {
        System.out.println(member.getName() + "이 엘릭서를 사용합니다.");
        System.out.println(member.getName() + "의 HP,MP 수치가 50%만큼 오릅니다.");
        member.setHp(member.getHp() + member.getMaxHp() / 2);
        member.setMp(member.getMp() + member.getMaxMp() / 2);
        increasePotionToxic(member);
    }
}

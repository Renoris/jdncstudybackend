...
public class JdncClassTest1 {
    ...
    private static void analyzingMember(Member[] members) {
        for (int i = 0; i < members.length; i++) {
            if (members[i] instanceof AdultMember) {
                System.out.printf("%s 의 타입은 AdultMember\n", members[i].getName());
            } else if (members[i] instanceof ChildrenMember) {
                System.out.printf("%s 의 타입은 ChildrenMember\n", members[i].getName());
            }
        }
